...
function spikelingFunctions(filename)
   ...
   eventPlot = plotEventsQuest();
   if dat.stim.stimPres==1&& eventPlot==1 % if stimulus present, plot stimulus things
    dat = eventThings(dat); 
    plotEvents(dat)
   end
   dat = sta(dat);
...
function eventPlot = plotEventsQuest()
%%ask if you want to plot event stuff
    answer = questdlg('Would you like to plot event-triggered superposition (not recommended for large datasets)?', ...
	'Plot', ...
	'Yes', 'No','No');
    % Handle response
    switch answer
...
function plotSTA(dat);

 %%
    
    figure('pos',[900 100 200 600])
    ...
    plot(dat.staT,dat.sta,'k')
    
    set(gca,'fontsize',8)
    ylabel('Light','fontsize',10)
    box off
    
    
    subplot(2,1,2)
    set(gca,'Xdir','reverse')
    plot(dat.staT,dat.staCurr,'k')
    set(gca,'fontsize',8)
   ...
   % subplot(4,1,4)
   % set(gca,'Xdir','reverse')
   % plot(dat.staT,dat.subStaCurr,'k')
   % set(gca,'fontsize',8)
   % ylabel('Current','fontsize',10)
    set(gca,'fontsize',8)
...
function dat = subThresh(dat);
    %% subthreshold STAA
    
    spikeThresh = -55; % THreshold to remove spiking activity
    subThresh = 1; % Threshold for differentiated voltage
    nPts = length(dat.v);
    vDiff = zeros(nPts,1);
    vDiff(1) = 0;
    % Differentiate
    for (i=2:nPts)
        vDiff(i) = dat.v(i)-dat.v(i-1);
    end
    % Remove spikes
    for (i=1:nPts)
        if dat.v(i)> spikeThresh
            vDiff(i) = 0;
        end
    end
    ...
    % COmpute subSTA
    backTime = .5;
    backInds = round(backTime/.004);
    subSTA = zeros(backInds+1,1);
    subStaCurr = subSTA;
    nGood = 0;
    for i=2:nPts-1
        if vDiff(i) > subThresh && vDiff(i-1) < subThresh && i > backInds % when diff crosses subthresh
            subSTA = subSTA + (dat.stimState(i-backInds:i).*vDiff(i)); % scale by vlues
            subStaCurr = subStaCurr + dat.totC(i-backInds:i).*vDiff(i);
            nGood = nGood +vDiff(i);
        end
    end
    %average and save
    subSTA = subSTA /nGood;
    subStaCurr = subStaCurr / nGood;
    dat.subSTA = subSTA;
    dat.subStaCurr = subStaCurr;
end
...
function dat = sta(dat);
    ...
    backTime = .5;
    backInds = round(backTime/.004);
    sta = zeros(backInds+1,1);
    staCurr  = zeros(backInds+1,1);
    nGood = 0;
    for i=1:length(dat.spikeInd)
        if (dat.spikeInd(i) > backInds)
            sta =sta + dat.stimState(dat.spikeInd(i)-backInds:dat.spikeInd(i));
            staCurr = staCurr + dat.totC(dat.spikeInd(i)-backInds:dat.spikeInd(i));
            nGood = nGood+1;
        end
    end
    sta = sta/ nGood;
    staCurr = staCurr/ nGood;
    dat.sta = sta;
    dat.staT = -.5:.004:0;
    dat.staCurr = staCurr;
...
function plotEvents(dat)
    disp('Plotting Event-Based Things')
