...
float timestep_ms   = 0.1;  // default 0.1, so the model runs at 1 kHz at default (0.1 ms steps). Changing this will speed up / slow down everything. This can be set a lot faster if desired, but individual functions will start to fail so test carefully
...
int   nModes = 5; // set this to number of entries in each array. Entries 1 define Mode 1, etc..
  ...
  // Iziekevich model parameters - for some pre-tested behaviours from the original paper, see bottom of the script
...
int   Array_DigiOutMode[] =  {       2,      2,       2,      2,       2  }; // PORT 1 setting. 0: Synapse 1 In, 1: Stimulus out, 2: 50 Hz binary noise out (for reverse correlation)
