...
public:
	...
	MCP23S08(const uint8_t csPin,const uint8_t haenAdrs,uint32_t spispeed);
	MCP23S08(const uint8_t csPin,const uint8_t haenAdrs,uint32_t spispeed, SPIClass *spi);

	MCP23S08();
	void			postSetup(const uint8_t csPin,const uint8_t haenAdrs,SPIClass *spi,uint32_t spispeed=0); //used with other libraries only
	virtual void 	begin(bool protocolInitOverride=false); //protocolInitOverride=false will not init the SPI	

	
	void 			gpioPinMode(uint8_t mode);					//set all pins to INPUT or OUTPUT
	void 			gpioPinMode(uint8_t pin, uint8_t mode);		//set a unique pin as INPUT or OUTPUT
